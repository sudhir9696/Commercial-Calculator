"""Commercial Math — CCIM 101 calculators.

Sub-page of the Streamlit MPA. The 6 tabs (TVM, DCF / Wealth, APOD, Multi-Year
Pro Forma, Financing, Leveraged Pro Forma) are inlined here so each calculator
keeps its own widget state and computations within the tab block.
"""
import streamlit as st
import numpy_financial as npf
import pandas as pd

st.title("📐 Commercial Math")
st.caption(
    "CCIM 101 calculators — Universal TVM, DCF / Wealth Accumulation, APOD, "
    "Multi-Year Pro Forma, Financing, Leveraged Pro Forma."
)
st.markdown("---")

# Note: `tab_screener` here is the APOD (Year 1) tab — the variable name is
# preserved from the original tvm.py so the inline code below still resolves.
...
with tab_dcf:
    ...
    if st.button("Run Wealth Analysis", type="primary"):
        ...
        # ==========================================
        st.markdown("---")
        st.markdown("### ðŸ§® Mathematical Proofs")
    ...
    st.divider() # Adds a nice gray line across the screen
    
    st.header("âš™ï¸ Capital Accumulation Comparison")
    ...
    if st.button("Calculate Capital Accumulation", type="primary"):
        ...
        def calc_irr(cfs):
            ...
            for _ in range(100):
                ...
                if abs(rate_new - rate) < 1e-6: return rate_new * 100
                rate = rate_new
            return rate * 100
            
        irr_a = calc_irr(inv_a)
        irr_b = calc_irr(inv_b)
        
        # 2. Step 1: Reinvest positive cash flows
        accum_a = sum(inv_a[t] * ((1 + r) ** (n - t)) for t in range(1, n + 1))
        accum_b = sum(inv_b[t] * ((1 + r) ** (n - t)) for t in range(1, n + 1))
        
        # 3. Step 2: Size Disparity Adjustment
        init_a = abs(inv_a[0])
        init_b = abs(inv_b[0])
        max_init = max(init_a, init_b)
        ...
        if init_a < init_b:
            diff = init_b - init_a
            external_inv_a = diff * ((1 + r) ** n)
            adj_accum_a += external_inv_a
        elif init_b < init_a:
            diff = init_a - init_b
            external_inv_b = diff * ((1 + r) ** n)
            adj_accum_b += external_inv_b
            
        # 4. Step 3: Compound Growth Rate (MIRR)
        cagr_a = ((adj_accum_a / max_init) ** (1 / n)) - 1 if max_init > 0 else 0
        cagr_b = ((adj_accum_b / max_init) ** (1 / n)) - 1 if max_init > 0 else 0
        
        winner = "Investment A" if cagr_a > cagr_b else "Investment B"
        
        # --- DISPLAY RESULTS ---
        st.markdown("---")
        st.markdown("### ðŸ“Š Capital Accumulation Results")
        
        c_r1, c_r2 = st.columns(2)
        with c_r1:
            st.info(f"#### Investment A")
            st.metric("Standard Flawed IRR", f"{irr_a:.2f}%")
            st.metric("Raw Accumulation (Step 1)", f"${accum_a:,.0f}")
            if external_inv_a > 0:
                st.caption(f"+ ${external_inv_a:,.0f} from external safe investment")
            st.metric("Adjusted Total Accumulation", f"${adj_accum_a:,.0f}")
            st.success(f"**Annual Growth Rate:** {cagr_a * 100:.2f}%")
            
        with c_r2:
            st.info(f"#### Investment B")
            st.metric("Standard Flawed IRR", f"{irr_b:.2f}%")
            st.metric("Raw Accumulation (Step 1)", f"${accum_b:,.0f}")
            if external_inv_b > 0:
                st.caption(f"+ ${external_inv_b:,.0f} from external safe investment")
            st.metric("Adjusted Total Accumulation", f"${adj_accum_b:,.0f}")
            st.success(f"**Annual Growth Rate:** {cagr_b * 100:.2f}%")
            
        st.markdown(f"### ðŸ† Recommendation: **{winner}** maximizes your wealth.")
...
with tab_screener:
   ...
   # --- SECTION 3: OPERATING EXPENSES (DYNAMIC) ---
    ...
    st.markdown("---")
    st.markdown("## ðŸ“„ Annual Property Operating Data (APOD)")
    ...
    for name, amount in all_expenses:
        if amount > 0:
            apod_data.append({"Line Item": f"   â€¢ {name}", "Subtotal": f"(${amount:,.0f})", "Total": ""})
...
with tab_proforma:
    ...
    st.markdown("---")
    st.markdown("### ðŸ“Š Cash Flow Analysis Worksheet (Operations)")
    ...
    st.markdown("---")
    st.markdown("### ðŸšª Disposition & Alternative Cash Sales Worksheet")
    ...
    st.markdown("---")
    st.markdown("### ðŸ“ˆ Investment Return Metrics")
...
with tab_financing:
    ...
    st.markdown("### ðŸ“Š Loan Sizing Analysis")
    
    col_res1, col_res2 = st.columns(2)
    with col_res1:
        st.info(f"**1. Loan Amount using LTV Ratio**\n\n"
                f"Value: \${prop_value:,.0f} Ã— LTV: {max_ltv}%\n\n"
    ...
    st.markdown("---")
    st.markdown("### ðŸ’¸ Effective Cost of Funds (Lender's Yield)")
    ...
    st.success(f"### ðŸŽ¯ Before-Tax Effective Cost of Funds: {eff_rate_annual:.2f}%")
...
with tab_Leverage_Pro_forma:
    st.header("ðŸ¢ Leveraged Investment Analysis")
    ...
    p_loan_term = c_m5.number_input("Loan Term (Years)", value=5, step=1) 
    ...
    st.markdown("### ðŸ“Š Financing Summary")
    ...
    st.markdown("### ðŸ“ˆ Cash Flow Analysis Worksheet (After-Tax)")
    ...
    st.markdown("### ðŸšª Sale Proceeds Before-Tax")
    ...
    st.markdown("### ðŸ§¾ Sale Proceeds After-Tax (Breakdown)")
    ...
    st.markdown("### ðŸ† Key Return Metrics")
    ...
    st.markdown("### âš–ï¸ Impact of Leverage Comparison")
    comp_data = {
